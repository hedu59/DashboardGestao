...
export class WizardComponent implements AfterContentInit {
  ...
  public next(): void {
    if (this.hasNextStep) {
      ...
      let aba =  this.activeStepIndex+1;
      localStorage.setItem('abaAtiva', aba.toString())
  ...
  public clickAba(): void {
    if (this.hasNextStep) {
      const nextStep: WizardStepComponent = this.steps[this.activeStepIndex];
      let aba =  this.activeStepIndex;
      localStorage.setItem('abaAtiva', aba.toString())
      this.activeStep.onNext.emit();
      this.activeStep = nextStep;
    }
